    ...
    <a href="{{ route('notifications.go', $notification->id) }}"
       ...
       class="px-4 py-4 hover:bg-slate-50 transition-colors flex items-center space-x-4 relative border-b border-slate-50/50 {{ $notification->read_at ? '' : 'bg-blue-50/40' }}">
        ...
        <div class="relative shrink-0">
            <img src="https://ui-avatars.com/api/?name={{ urlencode($notification->data['user_name'] ?? 'U') }}&background=random&color=fff" class="w-14 h-14 rounded-full object-cover border border-slate-100">
            <div class="absolute -bottom-1 -right-1 p-1.5 rounded-full ring-2 ring-white shadow-sm {{ ($notification->data['type'] ?? '') == 'like' ? 'bg-red-500' : 'bg-green-500' }}">
